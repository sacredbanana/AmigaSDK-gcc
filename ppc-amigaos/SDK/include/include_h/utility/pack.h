...
#define PK_BITNUM2(flg) \
        ((flg < 0x100 ? PK_BITNUM1(flg) : 8+PK_BITNUM1(flg >> 8)))

#define PK_BITNUM(flg) \
        ((flg < 0x10000 ? PK_BITNUM2(flg) : 16+PK_BITNUM2(flg >> 16)))

#define PK_WORDOFFSET(flg) ((flg) < 0x100 ? 1 : 0)

#define PK_LONGOFFSET(flg) \
        ((flg) < 0x100  ? 3 : (flg) < 0x10000 ? 2 : (flg) < 0x1000000 ? 1 : 0)
 ...
 *         PACK_ENTRY     (GA_Dummy, GA_Left,      Gadget, LeftEdge,
 *                         PKCTRL_WORD |PKCTRL_PACKUNPACK),
 *         PACK_ENTRY     (GA_Dummy, GA_Top,       Gadget, TopEdge,
 *                         PKCTRL_WORD |PKCTRL_PACKUNPACK),
 *         PACK_ENTRY     (GA_Dummy, GA_Width,     Gadget, Width,
 *                         PKCTRL_UWORD|PKCTRL_PACKUNPACK),
 *         PACK_ENTRY     (GA_Dummy, GA_Height,    Gadget, Height,
 *                         PKCTRL_UWORD|PKCTRL_PACKUNPACK),
 *         PACK_WORDBIT   (GA_Dummy, GA_RelVerify, Gadget, Activation,
 *                         PKCTRL_BIT  |PKCTRL_PACKUNPACK, GACT_RELVERIFY),
 *         PACK_ENDTABLE
 *    };
 */
...
#define PACK_ENTRY(tagbase,tag,type,field,control) \
        (control | ((tag-tagbase) << 16L) | PK_CALCOFFSET(type,field))

#define PACK_BYTEBIT(tagbase,tag,type,field,control,flags) \
        ( control                   |                      \
          ((tag-tagbase) << 16L)    |                      \
          PK_CALCOFFSET(type,field) |                      \
          (PK_BITNUM(flags) << 13L) )

#define PACK_WORDBIT(tagbase,tag,type,field,control,flags)     \
        ( control                                            | \
          ((tag-tagbase) << 16L)                             | \
          (PK_CALCOFFSET(type,field) + PK_WORDOFFSET(flags)) | \
          ((PK_BITNUM(flags)&7) << 13L)                      )

#define PACK_LONGBIT(tagbase,tag,type,field,control,flags)     \
        ( control                                            | \
          ((tag-tagbase) << 16L)                             | \
          (PK_CALCOFFSET(type,field) + PK_LONGOFFSET(flags)) | \
          ((PK_BITNUM(flags)&7) << 13L)                      )
