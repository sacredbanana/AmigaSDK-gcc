...
/* New for V50: dri_Screen, dri_Prefs. */
    struct Screen   *dri_Screen;      /* pointer to associated screen
                                       ...
                                       */
    APTR             dri_Prefs;       /* opaque handle to GUI settings
                                       * Will be NULL if DRI_VERSION < 3
                                       */
    ULONG            dri_Reserved[3]; /* avoid recompilation ;^) */
};

#define DRIF_NEWLOOK    0x00000001L /* specified SA_Pens, full treatment   */
#define DRIF_NEWDISABLE 0x00000002L /* fancy look for disabled items (V50) */
#define DRIF_REALSHADE  0x00000004L /* realistic bitmap shading (V50)      */
...
#define BARCONTOURPEN         (0x000C) /* contour above screen-bar            */
#define FOREGROUNDPEN         (0x000D) /* inside of unselected gadgets        */
#define FORESHINEPEN          (0x000E) /* bright edges of unselected gadgets  */
#define FORESHADOWPEN         (0x000F) /* dark edges of unselected gadgets    */
#define FILLSHINEPEN          (0x0010) /* bright edges for FILLPEN            */
#define FILLSHADOWPEN         (0x0011) /* dark edges for FILLPEN              */
#define INACTIVEFILLPEN       (0x0012) /* inactive window borders fill        */
#define INACTIVEFILLTEXTPEN   (0x0013) /* text over INACTIVEFILLPEN           */
#define INACTIVEFILLSHINEPEN  (0x0014) /* bright edges for INACTIVEFILLPEN    */
#define INACTIVEFILLSHADOWPEN (0x0015) /* dark edges for INACTIVEFILLPEN      */
#define DISABLEDPEN           (0x0016) /* background of disabled elements     */
#define DISABLEDTEXTPEN       (0x0017) /* text of disabled string gadgets     */
#define DISABLEDSHINEPEN      (0x0018) /* bright edges of disabled elements   */
#define DISABLEDSHADOWPEN     (0x0019) /* dark edges of disabled elements     */
#define MENUBACKGROUNDPEN     (0x001A) /* background of menus                 */
#define MENUTEXTPEN           (0x001B) /* normal text in menus                */
#define MENUSHINEPEN          (0x001C) /* bright edges of menus               */
#define MENUSHADOWPEN         (0x001D) /* dark edges of menus                 */
#define SELECTPEN             (0x001E) /* background of selected items        */
#define SELECTTEXTPEN         (0x001F) /* text of selected items              */
#define SELECTSHINEPEN        (0x0020) /* bright edges of selected items      */
#define SELECTSHADOWPEN       (0x0021) /* dark edges of selected items        */
#define GLYPHPEN              (0x0022) /* system gadget glyphs, outlines      */
#define GLYPHFILLPEN          (0x0023) /* system gadget glyphs, colored areas */
#define INACTIVEGLYPHPEN      (0x0024) /* system gadget glyphs, inact. windows*/
#define RESERVEDPEN           (0x0025) /* reserved - don't use                */
#define GADGETPEN             (0x0026) /* gadget symbols (arrows, cycle, etc.)*/
#define TITLEPEN              (0x0027) /* title of gadget groups              */
#define HALFSHINEPEN          (0x0028) /* half-bright edge on 3D objects      */
#define HALFSHADOWPEN         (0x0029) /* half-dark edge on 3D objects        */
#define FLATBORDERPEN         (0x002A) /* flat (non-3D) borders and frames    */
#define FILLFLATPEN           (0x002B) /* flat outlines of active windows     */
#define INACTIVEFILLFLATPEN   (0x002C) /* flat outlines of inactive windows   */

#define NUMDRIPENS            (0x002D)
...
#define SA_Reserved2         (SA_Dummy + 0x002b)
            /* Reserved for private Intuition use. V50. */

#define SA_ActiveWindow      (SA_Dummy + 0x002c)
            /* the active window of the screen. V50. */

#define SA_MaxWindowBox      (SA_Dummy + 0x002d)
                        /* (struct IBox *). V50. PRIVATE */

#define SA_Reserved3         (SA_Dummy + 0x002e)
            /* Reserved for private Intuition use. V51. */

#define SA_Compositing       (SA_Dummy + 0x002f)
            /* When TRUE, compositing mode is enabled for
             * layers of this screen. Set to FALSE to force
             * traditional layer handling. Set to ~0UL (or
             * don't pass this tag at all) to respect user
             * preferences for this attribute. V53. */

#define SA_WindowDropShadows (SA_Dummy + 0x0030)
            /* Allow drop shadows on window sides, if requested.
             * Set to FALSE to prevent shadows from ever being
             * drawn. This tag is ignored if compositing is not
             * enabled for the screen. Defaults to TRUE. V53. */
...
#define OSERR_BADBITMAP    (10) /* Custom bitmap not displayable (V51) */
