;/* SpeedBar Example
gcc -o SpeedBar speedbarexample.c -lauto
quit
*/
...
BOOL make_speedbar_list(struct List *, struct Image *, char **);
...
BOOL make_speedbar_list(struct List *list, struct Image *images, char **help)
{
	struct Node *node;
				...
				WINDOW_Layout, IIntuition->NewObject(NULL, "layout.gadget",
					...
					LAYOUT_AddChild, IIntuition->NewObject(NULL, "layout.gadget",
						...
						LAYOUT_AddImage, IIntuition->NewObject(NULL, "label.image",
							LABEL_Text, "The speedbar supports a scrolling list of image\n",
							LABEL_Text, "buttons. When more buttons are available than\n",
							LABEL_Text, "the visible space, you may scroll the list by\n",
							LABEL_Text, "pressing the SHIFT key while clicking the LMB\n",
							LABEL_Text, "and dragging the mouse. You can also use the\n",
							LABEL_Text, "scrollwheel to bring hidden buttons into view.\n",
							LABEL_Text, "The speedbar also supports several options\n",
							LABEL_Text, "such boolean buttons and MX groups!\n",
						TAG_DONE),
					TAG_DONE),
