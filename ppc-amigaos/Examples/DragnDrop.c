...
DISPATCHER(FieldsList_Dispatcher)
{
	switch(msg->MethodID)
	{
...
DISPATCHER(ChooseFields_Dispatcher)
{
	switch(msg->MethodID)
	{
...
{
	CL_FieldsList    = MUI_CreateCustomClass(NULL,MUIC_List ,NULL,sizeof(struct FieldsList_Data),ENTRY(FieldsList_Dispatcher));
	CL_ChooseFields  = MUI_CreateCustomClass(NULL,MUIC_Group,NULL,sizeof(struct ChooseFields_Data ),ENTRY(ChooseFields_Dispatcher ));

	if (CL_FieldsList && CL_ChooseFields)
	{
		// set up a specific name for our classes
		CL_FieldsList->mcc_Class->cl_ID = (ClassID)"DragnDrop/FieldList";
		CL_ChooseFields->mcc_Class->cl_ID = (ClassID)"DragnDrop/ChooseFields";
...
int main(UNUSED int argc, UNUSED char *argv[])
{
	...
	app = ApplicationObject,
		...
		MUIA_Application_Version    , "$VER: DragnDrop 21.3 (08.02.2018)",
		...
		SubWindow, window = WindowObject,
			...
			WindowContents, VGroup,
				...
				Child, ColGroup(2),
					Child, TextObject, TextFrame, MUIA_Background, MUII_TextBack, MUIA_Text_Contents, "\33cList without\nmultiple selection.", End,
					Child, TextObject, TextFrame, MUIA_Background, MUII_TextBack, MUIA_Text_Contents, "\33cList with\nmultiple selection.", End,
					Child, lv = ListObject,
						InputListFrame,
	...
	if (argc==2 && !strnicmp(argv[1],"bench",5))
	{
		...
		c=clock()-c;
		printf("list scroll benchmark took %ld ticks\n",c);
	}
	else
