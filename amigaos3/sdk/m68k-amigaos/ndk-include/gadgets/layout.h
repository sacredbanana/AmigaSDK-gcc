...
#define LM_ADDCHILD (0x540001UL) /* OS4ONLY */

struct lmAddChild /* OS4ONLY */
{
    ULONG            MethodID;
...
#define LM_ADDIMAGE (0x540002UL) /* OS4ONLY */

struct lmAddImage /* OS4ONLY */
{
    ULONG            MethodID;
	struct Window *  lm_Window;      /* Window pointer (may be NULL) */
...
#define LM_REMOVECHILD (0x540003UL) /* OS4ONLY */

struct lmRemoveChild /* OS4ONLY */
{
    ULONG           MethodID;
	struct Window * lm_Window;   /* Window pointer (may be NULL) */
...
#define LM_MODIFYCHILD (0x540004UL) /* OS4ONLY */

struct lmModifyChild /* OS4ONLY */
{
    ULONG           MethodID;
	struct Window *  lm_Window;      /* Window pointer (may be NULL) */
...
#define INTERSPACING 1		/* virtual pixels (strokes) */
...
/* LAYOUT_HelpHit and LAYOUT_HelpGadget used to exist but are no longer
 * sent out (notify) as it was always an internal thing and very buggy
 */
#define LAYOUT_HelpHit			(LAYOUT_Dummy+40) /* OS4ONLY */
#define LAYOUT_HelpGadget		(LAYOUT_Dummy+41) /* OS4ONLY */
...
#define LAYOUT_Inverted			(LAYOUT_Dummy+42)
	 ...
	 * It's also highly useful for dynamic layout, inserting the
...
#define LAYOUT_WeightBar		(LAYOUT_Dummy+43)
	/* See CHILD_WeightBar below.
	 */

#define LAYOUT_NoLayout			(LAYOUT_Dummy+44) /* V45 */
	/* (BOOL) Do not layout the children. All children must have valid
	coordinates. Don't use weight bars (CHILD_WeightBar). */

#define LAYOUT_LayoutBackFill		(LAYOUT_Dummy+45) /* OS4ONLY */

#define LAYOUT_LayoutMagic		(LAYOUT_Dummy+46) /* OS4ONLY */

#define LAYOUT_CharSet			(LAYOUT_Dummy+47) /* OS4ONLY */

#define LAYOUT_Immediate		(LAYOUT_Dummy+48) /* OS4ONLY */

#define LAYOUT_DoImmediate		(LAYOUT_Dummy+49) /* OS4ONLY */
...
#define CHILD_WeightBar			(LAYOUT_Dummy+43)
	/* (BOOL) Place a user adjustable weighting bar after the child
	 * This is the same tag value as the wrongly-named LAYOUT_WeightBar
	 * ClassAct 2.1 - V43.1
	 */
...
#define PAGE_NoDispose			(PAGE_Dummy+7)
	/* (BOOL) v53.22 and V47 */
