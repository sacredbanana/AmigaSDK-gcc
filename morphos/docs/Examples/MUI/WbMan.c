...
#define MUI_HOOK(n, y, z) \
	...
	static const struct Hook n##hook = { { 0, 0}, (void *)&n, (void *)&n##_GATE2 }; \
	static LONG n##_GATE2(struct Hook *h, y, z)
		...
		if (new_file_name = AllocMem(MAXNAMELEN, MEMF_ANY))
		{
			...
			for (i=0; ; i++)
			{
				...
				if (*(line+4) == 'b')
				{
					...
					if (*(line+2) == '8')
					{
						sprintf(file_name, "SYS:WBStartup/%s.noinfo", line+5);
						sprintf(new_file_name, "SYS:WBStartup/%s.info", line+5);
					}

					/*** turn status to inactive ***/
					else
					{
						sprintf(file_name, "SYS:WBStartup/%s.info", line+5);
						sprintf(new_file_name, "SYS:WBStartup/%s.noinfo", line+5);
					}
...
BOOL open_string_request(char *title_text, ULONG string_len)
{
...
void edit_tooltypes_entry(void)
{
	...
	if (file_name = AllocMem(MAXNAMELEN, MEMF_ANY))
	{
		...
		sprintf(file_name, "SYS:WBStartup/%s", line+5);
		...
		else
		{
			...
			temp_dobj_flag = TRUE;
			sprintf(file_name, "copy SYS:WBStartup/%s.noinfo T:WbMan_Temp.info", line+5);
			if (Execute(file_name, (BPTR)NULL, (BPTR)NULL) &&
				(disk_object = GetDiskObjectNew("T:WbMan_Temp"))
				)
				edit_tooltypes_diskobject(disk_object, line+5);
			else
			{
...
void save_tooltypes(void)
{
	if (disk_object)
	{
		...
		if (file_name = AllocMem(MAXNAMELEN, MEMF_ANY))
		{
			...
			old_tooltypes = (char **)disk_object->do_ToolTypes;
			...
			if (new_tooltypes = AllocMem((nr+1) << 2, MEMF_ANY))
			{
				...
				*l = NULL;
				...
				/*** new_data ***/
				disk_object->do_ToolTypes = (STRPTR *)new_tooltypes;
				...
				if (temp_dobj_flag)
				{
					PutDiskObject("T:WbMan_Temp", disk_object);

					sprintf(file_name, "copy T:WbMan_Temp.info SYS:WBStartup/%s.noinfo", line+5);
			...
			disk_object->do_ToolTypes = (STRPTR *)old_tooltypes;
...
MUI_HOOK(tools_list_con, APTR mem_pool, struct FileInfoBlock *fib)
{
		...
		if (entry = AllocMem(MAXNAMELEN, MEMF_ANY))
		{
			...
			if (MatchPatternNoCase(active_pattern, file_name))
			{
				...
				*(file_name + strlen(file_name) - 5) = 0;
				...
				{
					sprintf(entry, "8" ePW eN "%s", file_name);
					return((ULONG)entry);
				}
			}
			...
			else if (MatchPatternNoCase(inactive_pattern, file_name))
			{
				...
				*(file_name + strlen(file_name) - 7) = 0;
				...
				if (check_tool_name(file_name))
				{
					sprintf(entry, "2" ePB eN "%s", file_name);
					return((ULONG)entry);
				}
			}
...
MUI_HOOK(tools_list_des, APTR mem_pool, char *line)
{
...
MUI_HOOK(tools_list_dsp, char **array, char *line)
{
	*array = line+1;
	return(0);
}

MUI_HOOK(tools_list_cmp, char *line1, char *line2)
{
...
MUI_HOOK(select_rx, APTR appl, ULONG *arg)
{
	char *pattern;
...
static struct MUI_Command arexx_list[] =
{
	...
	{"select",		"PATTERN/A",		1,			&select_rxhook},
	{"activate",	MC_TEMPLATE_ID,		ID_ACTIVATE,	NULL},
	{"deactivate",	MC_TEMPLATE_ID,		ID_DEACTIVATE,	NULL},
	{"toggle",		MC_TEMPLATE_ID,		ID_TOGGLE,		NULL},
	{"restore",		MC_TEMPLATE_ID,		ID_RESTORE,		NULL},
...
static const struct NewMenu menu_list[] =
{
	...
	{ NM_ITEM,	"Rescan",			"S",0,	0,	(APTR) ID_RESCAN			},
	{ NM_ITEM,	NM_BARLABEL,		0,	0,	0,	0							},
	{ NM_ITEM,	"Perform",			"P",0,	0,	(APTR) ID_PERFORM			},
	{ NM_ITEM,	"Quit",				"Q",0,	0,	(APTR) ID_QUIT				},


	{ NM_TITLE,	"Tools",			0,	0,	0,	0							},

	{ NM_ITEM,	"Select All",		"A",0,	0,	(APTR) ID_SELECT_ALL		},
	{ NM_ITEM,	"Select None",		"N",0,	0,	(APTR) ID_SELECT_NONE		},
	{ NM_ITEM,	"Select Pattern",	"E",0,	0,	(APTR) ID_SELECT_PATTERN	},
	{ NM_ITEM,	NM_BARLABEL,		0,	0,	0,	0							},
	{ NM_ITEM,	"Activate",			"V",0,	0,	(APTR) ID_ACTIVATE			},
	{ NM_ITEM,	"Deactivate",		"D",0,	0,	(APTR) ID_DEACTIVATE		},
	{ NM_ITEM,	"Toggle",			"T",0,	0,	(APTR) ID_TOGGLE			},
	{ NM_ITEM,	"Restore",			"R",0,	0,	(APTR) ID_RESTORE			},
	{ NM_ITEM,	NM_BARLABEL,		0,	0,	0,	0							},
	{ NM_ITEM,	"Edit Tooltypes",	"O",0,	0,	(APTR) ID_EDIT				},
...
int main(int argc, char *argv[])
{
	...
	app = ApplicationObject,
		...
		MUIA_Application_Version,			"$VER: WbMan 0.42 (27.8.93)",
		MUIA_Application_Copyright,			"© 1993 kMel, Klaus Melchior",
