...
#ifdef __MORPHOS__
#define CreateProcess(name,func) CreateNewProcTags(NP_CodeType, MACHINE_PPC, NP_Entry,func,NP_Name,name,TAG_DONE)
#else
#define CreateProcess(name,func) CreateNewProcTags(NP_Entry,func,NP_Name,name,TAG_DONE)
#endif
...
struct SubTask *SpawnSubTask(char *name,VOID (*func)(VOID),APTR data)
{
	...
	if (st = AllocVec(sizeof(struct SubTask),MEMF_PUBLIC|MEMF_CLEAR))
	{
		if (st->st_Reply = CreateMsgPort())
		{
			...
			if (st->st_Task = (struct Task *)CreateProcess(name,func))
			{
...
struct SubTask *InitSubTask(void)
{
	...
	*/
	...
	if (st->st_Port = CreateMsgPort())
	{
		...
		** of 0 after a Forbid(). This tells SubSpawnSubTask() that the
...
VOID ASM SAVEDS RenderFunc(VOID)
{
	...
	if (GfxBase = (struct GfxBase *)OpenLibrary("graphics.library",37)) /* dont share library pointers! */
	{
		if (st = InitSubTask())
		{
...
int main(int argc,char *argv[])
{
	...
	if (!(mcc = MUI_CreateCustomClass(NULL,MUIC_Area,NULL,sizeof(struct Data),DISPATCHER_REF(Mandel))))
		fail(NULL,"Could not create custom class.");

	app = ApplicationObject,
		MUIA_Application_Title      , "Class4",
		MUIA_Application_Version    , "$VER: Class4 20.168 (04.04.03)",
		MUIA_Application_Copyright  , "© 1993-97 Stefan Stuntz",
