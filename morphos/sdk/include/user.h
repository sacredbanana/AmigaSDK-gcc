...
/* internal structure used by __alloca() - 50.2 */
struct alloca_mem {
	struct alloca_mem *next;
	void              *addr;
	u_int             depth;
};
...
struct vfork_msg {
  ...
  int                   vm_plowerppc;   /* lower bound of parents stack */
  int                   vm_pupperppc;   /* upper bound of parents stack */
  int                   vm_clowerppc;   /* lower bound of childs stack */
  int                   vm_cupperppc;   /* upper bound of childs stack */
#endif
};
...
#define u_r13 u_a4 /* for ppc */
	...
	struct stackframe       *u_68k_stk_used;    /* used stackframes */
	struct stackframe       *u_68k_stk_spare;   /* spare stackframes */
	...
	struct stackframe       *u_ppc_stk_used;    /* used stackframes */
	struct stackframe       *u_ppc_stk_spare;   /* spare stackframes */
...
#ifdef NATIVE_MORPHOS
	uint32_t		u_68k_stk_usage_highest;
	uint32_t		u_ppc_stk_usage_highest;
#endif
	int			u_local_errno;
	int			u_local_h_errno;
...
enum
{
	IX_TT_NORMAL,  /* normal process */
	IX_TT_VFORK,   /* vfork()ed process before exec*() has been called */
	IX_TT_ASYNCH   /* pthread or similar asyncron thread */
};
...
#define SUSAGE_DONE     0x1000000       /* we already reported the stack usage */
#define EXECVECLOSE     0x10000000      /* flag to handle execve() + atexit stdio close suspend support */
#define SUSAGE_TRACKED  0x20000000      /* whether this process is tracking stack usage */
