...
int ix_select(int nfd, fd_set *ifd, fd_set *ofd, fd_set *efd,
	      const struct timeval *timeout, long *mask);
...
/* Return/set extra flags for the user struct
 */
...
#define IX_EF_RESERVED0         (1 << 0) /* Reserved, do not use */
...
struct user;
void ix_user_release_sockets(struct user *mu);
int ix_user_clone_parent(struct user *pu, struct user *mu);
void ix_user_cleanup(struct user *mu);
...
/* Internal function to initialize a mutex */
void i_mutex_init(struct ix_mutex *mutex);
