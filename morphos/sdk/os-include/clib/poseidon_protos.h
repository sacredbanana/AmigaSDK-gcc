...
/*
**	$Id: poseidon_protos.h,v 1.13 2008/09/15 21:36:25 piru Exp $
**
**	C prototypes. For use with 32 bit integers only.
**
**	Copyright © 2008 Chris Hodges
**	    All Rights Reserved
*/
...
APTR psdAllocPipe( APTR pd, struct MsgPort *mp, APTR pep );
...
APTR psdOpenStreamA( APTR pep, struct TagItem *taglist );
APTR psdOpenStream( APTR pep, ... );
...
VOID psdDebugSemaphores( VOID );
VOID psdHubClassScan( APTR pd );
APTR psdHubClaimAppBindingA( struct TagItem *taglist );
VOID psdHubReleaseDevBinding( APTR pd );
VOID psdHubReleaseIfBinding( APTR pif );
APTR psdAllocCfgForm( ULONG formid );
BOOL psdAddStringChunk( APTR pic, ULONG chunkid, STRPTR contents );
BOOL psdMatchStringChunk( APTR pic, ULONG chunkid, STRPTR contents );
STRPTR psdGetStringChunk( APTR pic, ULONG chunkid );
STRPTR psdOnlineReport( APTR formdata );
APTR psdFindDescriptorA( APTR pd, APTR pdd, struct TagItem *taglist );
APTR psdFindDescriptor( APTR pd, APTR pdd, ... );
APTR psdDoHubMethodA( APTR pd, ULONG methodid, APTR methoddata );
APTR psdDoHubMethod( APTR pd, ULONG methodid, ... );
APTR psdAllocRTIsoHandlerA( APTR pep, struct TagItem *taglist );
APTR psdAllocRTIsoHandler( APTR pep, ... );
VOID psdFreeRTIsoHandler( APTR prt );
LONG psdStartRTIso( APTR prt );
LONG psdStopRTIso( APTR prt );
ULONG psdBorrowLocksWait( struct Task *task, ULONG signals );
