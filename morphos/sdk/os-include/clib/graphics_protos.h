...
VOID SetCollision( ULONG num, VOID (*routine) __CLIB_PROTOTYPE((struct VSprite *gelA,struct VSprite *gelB)), struct GelsInfo *gelsInfo );
...
#if !defined(USE_INLINE_STDARG)
WORD WeighTAMatchTags(struct TTextAttr *reqTextAttr, struct TextAttr *targetTextAttr, ULONG Tag1, ...);
#endif
WORD WeighTAMatch(struct TTextAttr *reqTextAttr, struct TextAttr *targetTextAttr, struct TagItem *targetTags);
...
struct TextFont *OpenFontTagList(struct TextAttr *textAttr, struct TagItem *tags);
#if !defined(USE_INLINE_STDARG)
struct TextFont *OpenFontTags(struct TextAttr *textAttr, ULONG Tag1, ... );
#endif
