...
#if PLAT_MORPHOS
	virtual void Create_TTE(const FontParameters &fp);
	virtual void Release_TTE();
	BOOL ttengine;
#endif
...
protected:
	...
	/* non-calltip window handling calltips */
	struct List       lCalltips;
	struct WindowNode Node;

	/* calltip window */
	Object       *callTipWindow;
#endif
public:
	Window() : wid(0), cursorLast(cursorInvalid) {
#if PLAT_MORPHOS
	  muiObject = 0; bCalltip = false; callTipWindow = 0; muiApplication = 0; bCalltipAdded = false; parentWindow = NULL;
	  Initialize();
	  Node.self = this;
#endif
	}
	...
	void SetMuiObject(Object *ref) { muiObject = ref; };
	Object *GetMuiObject(void) const { return muiObject; };

	struct WindowNode *GetNode(void) { return &Node; };

	/* calltip handling */
	void SetIsCalltipWindow(bool bIsCalltip) { bCalltip = bIsCalltip; };
	bool GetIsCalltipWindow(void) const { return bCalltip; };

	void SetCalltipWindowObject(Object *calltip) { callTipWindow = calltip; };
	Object *GetCalltipWindowObject(void) { return callTipWindow; };

	void SetParentWindow(WindowPtr parent) { parentWindow = parent; };

	/* appending calltips to non-calltip windows */
	void AddCalltip(Window *win);
	void RemoveCalltip(Window *win);

	/* these are called on both types, either doing the job or forwarding to objs on the list */
	void ParentSetup(void);
	void ParentCleanup(void);
	void ParentShow(void);
	void ParentHide(void);
	void ParentWindowArranged(void);
...
public:
	...
	#ifdef PLAT_MORPHOS
	static PRectangle PRecToARec(PRectangle rc, void *areaobj);
	#endif
};
